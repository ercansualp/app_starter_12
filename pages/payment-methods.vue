  ...
  <div>
    ...
    <div class="relative">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <LoadingOverlay :isLoading="isUpdatingCardId === method.id" message="Güncelleniyor..." />
          <!-- Card Header -->
          <div>
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div class="bg-card rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col border border-border-color relative">
        <LoadingOverlay :isLoading="isProcessing" message="Ödeme yöntemi ekleniyor..." />
...
import NotificationToast from '~/components/NotificationToast.vue' // Import the new component
...
const loadingMessage = ref('Ödeme yöntemleri yükleniyor...')
...
const handleAddPaymentMethod = async () => {
  ...
  try {
    ...
    closeAddModal()
    showNotification('success', 'Ödeme yöntemi başarıyla eklendi!')
    
  } catch (error) {
    console.error('Ödeme yöntemi ekleme hatası:', error)
    showNotification('error', 'Ödeme yöntemi eklenirken bir hata oluştu. Lütfen tekrar deneyin.')
  } finally {
...
const togglePaymentMethodStatus = async (method) => {
  if (method.isDefault && method.isActive) {
    ...
    showNotification('info', 'Varsayılan ödeme yöntemini pasif hale getiremezsiniz. Önce başka bir ödeme yöntemini varsayılan yapın.')
    return
  }
  ...
  try {
    ...
    showNotification('success', `Ödeme yöntemi durumu başarıyla ${method.isActive ? 'aktifleştirildi' : 'pasifleştirildi'}!`)
  } catch (error) {
    console.error('Durum güncelleme hatası:', error)
    showNotification('error', 'Ödeme yöntemi durumu güncellenirken bir hata oluştu.')
  } finally {
  ...
  confirmationConfig.value = {
    ...
    onConfirm: async () => {
      ...
      try {
        ...
        showNotification('success', 'Ödeme yöntemi başarıyla varsayılan olarak ayarlandı!')
      } catch (error) {
        console.error('Varsayılan ayarlama hatası:', error)
        showNotification('error', 'Ödeme yöntemi varsayılan olarak ayarlanırken bir hata oluştu.')
      } finally {
        isUpdatingCardId.value = null
      }
    }
