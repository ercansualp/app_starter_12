    ...
    <div class="space-y-8">
      ...
      <div v-if="selectedTab === 'jobs'">
        ...
        <div class="relative bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005] mb-8">
          <LoadingOverlay :isLoading="jobsLoading" :message="jobsMessage" />
          <div class="p-6">
              ...
              <table class="w-full text-left">
                ...
                <tbody>
                  <tr v-if="!jobsLoading && filteredJobs.length === 0">
                    <td colspan="5" class="p-4 text-center text-text-secondary">Henüz Job bulunmamaktadır.</td>
                  </tr>
                  <tr 
                    v-else
                  ...
                  >
                    <LoadingOverlay :isLoading="isDeletingJobId === job.id || (isProcessing && confirmationConfig.onConfirm === deleteJob && confirmationConfig.targetId === job.id)" message="Siliniyor..." />
      ...
      <div v-if="selectedTab === 'job-instances'">
        ...
        <div class="relative bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005]">
          <LoadingOverlay :isLoading="instancesLoading" :message="instancesMessage" />
          <div class="p-6">
                  ...
                  <table class="w-full text-left">
                    ...
                    <tbody>
                      <template v-for="instance in paginatedInstances" :key="instance.id">
                        ...
                        >
                          <LoadingOverlay :isLoading="isDeletingInstanceId === instance.id || (isProcessing && confirmationConfig.onConfirm === deleteInstance && confirmationConfig.targetId === instance.id)" message="Siliniyor..." />
      ...
      <div v-if="selectedTab === 'job-logs'">
        <!-- Job Logs Card -->
        <div class="relative bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005]">
          <LoadingOverlay :isLoading="logsLoading" :message="logsMessage" />
          <div class="p-6">
...
// Loading messages
const jobsMessage = ref('Job verileri yükleniyor...')
const instancesMessage = ref('Job Instance verileri yükleniyor...')
const logsMessage = ref('Job Log verileri yükleniyor...')
...
const handleJobUpdated = async (updatedJobData) => {
  ...
  try {
    ...
    if (jobIndex !== -1) {
      ...
      showNotification('success', 'Job başarıyla güncellendi!');
    } else {
      showNotification('error', 'Güncellenecek Job bulunamadı.');
    }
    closeJobModal();
  } catch (error) {
    console.error('Job güncelleme hatası:', error);
    showNotification('error', 'Job güncellenirken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
  }
};
  ...
  try {
    ...
    if (job) {
      ...
      if (instanceIndex !== -1) {
        ...
        showNotification('success', 'Job Instance başarıyla güncellendi!');
      } else {
        showNotification('error', 'Güncellenecek Job Instance bulunamadı.');
      }
    }
    closeEditInstanceModal();
  } catch (error) {
    console.error('Job Instance güncelleme hatası:', error);
    showNotification('error', 'Job Instance güncellenirken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
  }
};
  ...
  try {
    ...
    if (job) {
      ...
      if (instanceToUpdate) {
        ...
        showNotification('success', `Job Instance başarıyla ${newStatus ? 'başlatıldı' : 'durduruldu'}!`);
      } else {
        showNotification('error', 'Job Instance bulunamadı.');
      }
    }
  } catch (error) {
    console.error('Job Instance durum güncelleme hatası:', error);
    showNotification('error', `Job Instance ${newStatus ? 'başlatılırken' : 'durdurulurken'} bir hata oluştu.`);
  } finally {
    isProcessing.value = false;
  }
};
...
const stopAllInstances = async (job) => {
  ...
  try {
    ...
    if (targetJob) {
      ...
      showNotification('success', `'${job.url}' adresine sahip Job'ın tüm instance'ları durduruldu.`);
    } else {
      showNotification('error', 'Job bulunamadı.');
    }
  } catch (error) {
    console.error('Tüm instance\'ları durdurma hatası:', error);
    showNotification('error', 'Tüm instance\'lar durdurulurken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
  }
};
...
const startAllInstances = async (job) => {
  ...
  try {
    ...
    if (targetJob) {
      ...
      showNotification('success', `'${job.url}' adresine sahip Job'ın tüm instance'ları başlatıldı.`);
    } else {
      showNotification('error', 'Job bulunamadı.');
    }
  } catch (error) {
    console.error('Tüm instance\'ları başlatma hatası:', error);
    showNotification('error', 'Tüm instance\'lar başlatılırken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
  }
};
...
const deleteJob = async (jobId) => {
  ...
  try {
    ...
    if (allJobsData.value.length < initialLength) {
      showNotification('success', 'Job başarıyla silindi!');
    } else {
      showNotification('error', 'Job bulunamadı veya silinemedi.');
    }
    ...
    showNotification('error', 'Job silinirken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
  ...
  try {
    ...
    if (job) {
      ...
      if (job.jobInstances.length < initialLength) {
        showNotification('success', 'Job Instance başarıyla silindi!');
      } else {
        showNotification('error', 'Job Instance bulunamadı veya silinemedi.');
      }
      ...
      showNotification('error', 'İlgili Job bulunamadı.');
    }
  } catch (error) {
    console.error('Job Instance silme hatası:', error);
    showNotification('error', 'Job Instance silinirken bir hata oluştu.');
  } finally {
    isProcessing.value = false;
